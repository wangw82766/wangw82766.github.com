...
$(document).ready(function(){
		...
		$(canvas2).bind('touchend',function(event){
		...
		$(img).bind("load",function(){
			...
			$(canvas_color).bind("touchmove",function(event){
		...
		$("#size_bar").bind("touchstart",function(event){
			...
			$(document).bind("touchmove",size_bar_move);
			$(document).bind("touchend",function unbind(event){
				$(document).unbind("touchmove",size_bar_move);
				$(document).unbind("touchend",unbind);
			});
		});

		$("#r_channel_bar").bind("touchstart",function(event){
			var thumb=$("#r_channel_thumb");
			var main_w=$(this).width();
			var mainLeft=$(this).offset().left;
			thumb.css("left",event.clientX-mainLeft-thumb.width()/2+"px");
			$(document).bind("touchmove",{c:"r"},channel_bar_move);
			$(document).bind("touchend",function unbind(event){
				canvas_rgb.r=0.5+$(thumb).position().left/main_w;
				change_channel();
				$(document).unbind("touchmove",channel_bar_move);
				$(document).unbind("touchend",unbind);
			});
		});
		$("#g_channel_bar").bind("touchstart",function(event){
			var thumb=$("#g_channel_thumb");
			var main_w=$(this).width();
			var mainLeft=$(this).offset().left;
			thumb.css("left",event.clientX-mainLeft-thumb.width()/2+"px");
			$(document).bind("touchmove",{c:"g"},channel_bar_move);
		...
		$("#b_channel_bar").bind("touchstart",function(event){
			var thumb=$("#b_channel_thumb");
			var main_w=$(this).width();
			var mainLeft=$(this).offset().left;
			thumb.css("left",event.clientX-mainLeft-thumb.width()/2+"px");
			$(document).bind("touchmove",{c:"b"},channel_bar_move);
			$(document).bind("touchend",function unbind(event){
